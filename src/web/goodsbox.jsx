...
import Search from '../web/goodsbox_search.jsx';
...
//页面名称
const PageName='goodsbox'+Date.parse(new Date());
...
const columns = [{
  ...
  title: '清点数',
  dataIndex: 'GBRP_CHECK_NUM'
},{
  title: '出入数',
  dataIndex: 'GBRP_COUNT_NUM'
},{
  title: '废弃数',
  dataIndex: 'GBRP_DISCARD_NUM'
},{
  title: '预存数',
  dataIndex: 'GBRP_SAVE_AMOUNT'
},{
  title: '卡箱类型',
  dataIndex: 'CTTP_CODE',
  ...
  title: '记录时间',
  dataIndex: 'GBRP_INPUT_DT',
  render(text, row, index) {
   return row.GBRP_INPUT_DT==""||row.GBRP_INPUT_DT==null?
        '暂无':commonFunction.formatTime(row.GBRP_INPUT_DT,"yyyy-MM-dd hh:mm:ss")
  }
},{
  title: '操作',
  key: 'operation',
    ...
    return (
      <div>
        ...
        <Modal ref="modal"
          ...
          title={'重新录入-卡箱'+this.props.GBRP_GOODSBOX_CODE}
...
const Goodsbox= React.createClass({
  ...
  fetchList(params = {}) {
    ...
    reqwest({
      url:web_config.http_request_domain+'/proc/goodsbox/history',
      method: 'POST',
      ...
      success: (result) => {
        ...
        pagination.total = result.data.O_T_GOODSBOX_RP_DETAIL.count;
        pagination.current = result.data.O_T_GOODSBOX_RP_DETAIL.currentPage;
        this.setState({
          loading: false,
          data: result.data.O_T_GOODSBOX_RP_DETAIL.data,
          pagination,
        });
      },
      error:()=>{
  ...
  fetchEdit(evtName,data){
    ...
    this.setState({ loading: true });
    reqwest({
      url:web_config.http_request_domain+'/proc/goodsbox/modify',
      method: 'POST',
      timeout :web_config.http_request_timeout,
      data:EditParams,
      crossOrigin: web_config.http_request_cross, //跨域
     ...
     <Row>
      ...
      <Col span="1" style={{marginLeft:-20}}>
        <Button type="primary" htmlType="submit" onClick={this.resetSearch} >重置</Button>
          ...
          <Col span="12" className="table-add-layer"><span className="table-add-btn Alink" onClick={this.handleSearch} >(卡箱编号缺失，错误，请点击此处)</span></Col>
     </Row>
