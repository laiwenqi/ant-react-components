...
import Search from '../web/cashbox_search.jsx';
import './cashbox.less';
...
//页面名称
const PageName='cashbox'+Date.parse(new Date());
...
const columns = [{
  ...
  title: '点钞金额(元)',
  dataIndex: 'CBRP_COUNT_AMOUNT'
},{
  title: '1元数量(张)',
  dataIndex: 'CBRP_COUNT_1'
},{
  title: '5元数量(张)',
  dataIndex: 'CBRP_COUNT_5'
},{
  title: '10元数量(张)',
  dataIndex: 'CBRP_COUNT_10'
},{
  title: '20元数量(张)',
  dataIndex: 'CBRP_COUNT_20'
},{
  title: '50元数量(张)',
  dataIndex: 'CBRP_COUNT_50'
},{
  title: '100元数量(张)',
  dataIndex: 'CBRP_COUNT_100'
},{
  title: '钞箱类型',
  dataIndex: 'CTTP_CODE',
  ...
  title: '记录时间',
  dataIndex: 'CBRP_INPUT_DT',
  render(text, row, index) {
   return row.CBRP_INPUT_DT==""||row.CBRP_INPUT_DT==null?
        '暂无':commonFunction.formatTime(row.CBRP_INPUT_DT,"yyyy-MM-dd hh:mm:ss")
  }
},{
  title: '操作',
  key: 'operation',
...
const Edit = React.createClass({
  ...
  render() {
    return (
      <div>
        <a type="primary" onClick={this.showModal} {...this.props}>重新录入</a>
        <Modal ref="modal"
          width="550"
          visible={this.state.visible}
          title={'重新录入-钞箱'+this.props.CBRP_CASHBOX_CODE}
...
//标签分页里面的整个内容
const Cashbox= React.createClass({
   getInitialState() {
    return {
      data: [],
      defaultFilter:{},
  ...
  fetchList(params = {}) {
    ...
    reqwest({
      url:web_config.http_request_domain+'/proc/cashbox/history',
      method: 'POST',
      ...
      success: (result) => {
        ...
        pagination.total = result.data.T_CASHBOX_RP_DETAIL.count;
        pagination.current = result.data.T_CASHBOX_RP_DETAIL.currentPage;
        this.setState({
          loading: false,
          data: result.data.T_CASHBOX_RP_DETAIL.data,
          pagination,
        });
      },
      error:()=>{
  ...
  fetchEdit(evtName,data){
    ...
    this.setState({ loading: true });
    reqwest({
      url:web_config.http_request_domain+'/proc/cashbox/modify',
      method: 'POST',
      timeout :web_config.http_request_timeout,
      data:EditParams,
      crossOrigin: web_config.http_request_cross, //跨域
  ...
  render() {
    if(this.state.Search.visible==true){
      return (<Search {...this.state.Search.params}/>)
    }
     ...
     <Row>
      ...
      <Col span="1" style={{marginLeft:-20}}>
        <Button type="primary" htmlType="submit" onClick={this.resetSearch} >重置</Button>
          ...
          <Col span="12" className="table-add-layer"><span className="table-add-btn Alink" onClick={this.handleSearch} >(钞箱编号缺失，错误，请点击此处)</span></Col>
     </Row>
